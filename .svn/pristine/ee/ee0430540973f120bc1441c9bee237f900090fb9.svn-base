...
namespace DONGSHIN
{
    public static class fx_errorCode
    {
        public static commonReturn read_all(SqlConnection pCON, string chk) //조회함수
        {
            ...
            if ( chk == "Y" )
            {
                str = @"select * from ERROR where use_chk = 'Y' order by server_id";
            }
            else
            {
                str = @"select * from ERROR order by server_id";
            }
        ...
        public static commonReturn findByMsg(SqlConnection pCON, string msg, string chk)
        {
            commonReturn return2 = new commonReturn();
            string str = "";

            if ( chk == "Y" )
                str = @"select * from ERROR where err_msg like '%' + @msg + '%' and use_chk = @chk";
            else
                str = @"select * from ERROR where err_msg like '%' + @msg + '%' ";
            ...
            try
            {
                ...
                pCMD.Parameters.Clear();
                pCMD.Parameters.AddWithValue("@msg", msg);
        ...
        public static commonReturn findByID(SqlConnection pCON, string id)
        {
            ...
            str = @"select * from ERROR where err_id = @err_id ";
            ...
            try
            {
                ...
                pCMD.Parameters.Clear();
                pCMD.Parameters.AddWithValue("@err_id", id);
                return2 = DBsql.SQLExecuteDS(pCON, pCMD);
            }
            catch ( Exception exception )
            {
        ...
        public static commonReturn delete(SqlConnection pCON, string id)
        {
            ...
            {
                SqlCommand pCMD = new SqlCommand();
                str = "delete from ERROR where err_id = @err_id";
                pCMD.CommandText = str;
                pCMD.Connection = pCON;
                pCMD.Parameters.Clear();
                pCMD.Parameters.AddWithValue("@err_id", id);
