...
namespace DONGSHIN
{
    public partial class ctl_machineCode : UserControl
    {
        public ctl_machineCode()
        {
            InitializeComponent();
        ...
        private DataSet ds_machineCode = new DataSet();
        private DataRow parentRow;
        private string machineID = "";
        ctl_machineEdit edit;

        private void ctl_machineCode_Load(object sender, EventArgs e)
        {
            userProvisioning("140000");
            edit = new ctl_machineEdit(this);
            setVisible(pan_sub, true);
            read();
        }

        private void ctl_machineCode_VisibleChanged(object sender, EventArgs e)
        {            
            if ( this.Visible == true )
                ctl_machineCode_Load(this, e); 
        }
        ...
        private void read()
        {
            ...
            Return = fx_machineCode.read_all(commonVar.DBconn,use);
           
            if ( Return.Message == "" )
            {
                ds_machineCode = Return.Dataset;
        ...
        private void conditionalRead()
        {
            ...
            Return = fx_machineCode.findByName(commonVar.DBconn, txt_name.Text, use);
        ...
        {
            if ( ds_machineCode.Tables.Count > 0 )
            {
                if ( ds_machineCode.Tables[0].Rows.Count > 0 )
                {
                    FormGridFilter FormFilter = new FormGridFilter();

                    FormFilter.Grid넘기기(ref ds_machineCode, ref gridView1);
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            ...
            if ( btn_update.Enabled == true )
            {
                ...
                try
                {
                    ...
                    {
                        machineID = parentRow["MACHINE_CODE"].ToString();
                        edit.setID(machineID);
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    string maptype = parentRow["MapType"].ToString();
                    if ( maptype.Length > 0 ) 
                    {
                        question = MessageBox.Show("기존에 등록된 메모리맵이 존재합니다.\n\r삭제하시겠습니까?", "삭제", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    }
                    else
                    {
                        question = MessageBox.Show("선택하신 자료를 삭제하시겠습니까?", "삭제", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    }
                    if ( question == DialogResult.Yes )
                    {
                        //이미지삭제
                        string path = Convert.ToString(parentRow["sb_img"]);
                        if ( path != "no image" )
                        {
                        ...
                        Return = fx_machineCode.delete(commonVar.DBconn, machineID);
        ...
        private void btn_dataconvert_Click(object sender, EventArgs e)
        {
            ...
            if ( btn_dataconvert.Enabled == true )
            {
                ...
                tmF.Grid받기(lbl_title.Text, "설비코드", gridView1);
                setVisible(gridControl1, false);
