...
namespace DONGSHIN
{
    public partial class ctl_resinCode : UserControl
    {
        public ctl_resinCode()
        {
            InitializeComponent();
        ...
        private DataSet ds_resinCode = new DataSet();
        private DataRow parentRow;
        private string resinID = ""; 

        private void ctl_resinCode_Load(object sender, EventArgs e)
        {
            userProvisioning("165000");
            edit = new ctl_resinEdit(this);
            setVisible(pan_sub, true);
            read();
        }

        private void ctl_resinCode_VisibleChanged(object sender, EventArgs e)
        {
            if ( this.Visible == true )
                ctl_resinCode_Load(this, e); 
        }
        ...
        private void btn_update_Click(object sender, EventArgs e)
        {
            if( btn_update.Enabled == true )
            {
                ...
                try
                {
                    ...
                    {
                        resinID = parentRow["sj_id"].ToString();
                        edit.setID(resinID);
                    }
        ...
        private void btn_delete_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if ( question == DialogResult.Yes )
                {
                    ...
                    Return = fx_resinCode.deleteResinRecord(commonVar.DBconn, resinID);
        ...
        private void btn_dataconvert_Click(object sender, EventArgs e)
        {
            if ( btn_dataconvert.Enabled == true )
            {
                ...
                tmF.Grid받기(lbl_title.Text, "수지코드", gridView1);
        ...
        {
            if ( ds_resinCode.Tables.Count > 0 )
            {
                if ( ds_resinCode.Tables[0].Rows.Count > 0 )
                {
                    FormGridFilter FormFilter = new FormGridFilter();

                    FormFilter.Grid넘기기(ref ds_resinCode, ref gridView1);
