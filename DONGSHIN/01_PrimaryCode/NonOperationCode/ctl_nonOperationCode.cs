...
namespace DONGSHIN
{
    public partial class ctl_nonOperationCode : UserControl
    {
        public ctl_nonOperationCode()
        {
            InitializeComponent();
            commonFX.setThisLanguage(this);
        }

        ctl_nonOperationEdit edit;

        private DataSet ds_nonOperationCode = new DataSet();
        private DataRow parentRow;
        private string nonOpID = "";

        private void ctl_nonOperationCode_Load(object sender, EventArgs e)
        {
            userProvisioning("186000");
            edit = new ctl_nonOperationEdit(this);
            setVisible(pan_sub, true);
            read();
        }

        private void ctl_nonOperationCode_VisibleChanged(object sender, EventArgs e)
        {
            if ( this.Visible == true )
                ctl_nonOperationCode_Load(this, e);
        }
        ...
        private void btn_dataconvert_Click(object sender, EventArgs e)
        {
            if ( btn_dataconvert.Enabled == true )
            {
                ...
                tmF.Grid받기(lbl_title.Text, "비가동코드", gridView1);
        ...
        private void read()
        {
            ...
            Return = fx_nonOperationCode.read_all(commonVar.DBconn);

            if ( Return.Message == "" )
            {
                ds_nonOperationCode = Return.Dataset;
        ...
        private void conditionalRead()
        {
            ...
            Return = fx_nonOperationCode.findByReason(commonVar.DBconn, txt_why.Text);
        ...
        {
            if ( ds_nonOperationCode.Tables.Count > 0 )
            {
                if ( ds_nonOperationCode.Tables[0].Rows.Count > 0 )
                {
                    FormGridFilter FormFilter = new FormGridFilter();

                    FormFilter.Grid넘기기(ref ds_nonOperationCode, ref gridView1);
